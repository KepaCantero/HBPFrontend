...
(function() {
  ...
  angular.module('noiseModelModule', []).directive('noiseModel', [
    'noiseModelService',
    'baseEventHandler',
    'simulationInfo',
    'roslib',
    function(noiseModelService, baseEventHandler, simulationInfo, roslib) {
      return {
        ...
        link: scope => {
          ...
          rosServiceGetNoiseParameters.callService(request0, function(result) {
            scope.selNoiseType = result.type_noise;
            if (scope.selNoiseType != '') {
              document.getElementById('oi' + scope.selNoiseType).checked = true;
            }
          ...
          scope.onMeanParameterChanged = function() {
            ...
            if (scope.mean < 0.0 || (scope.mean == 0.0 && checkZeroValueMean)) {
