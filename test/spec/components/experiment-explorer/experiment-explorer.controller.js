...
describe('Controller: ExperimentExplorerController', function() {
  ...
  var $httpBackend, $rootScope, $log, element, storageServer; // eslint-disable-line no-unused-vars

  var experimentProxyService, $q, experimentsFactory; // eslint-disable-line no-unused-vars
  ...
  it('should download file successfully', function() {
    var controller = loadExperiments();
    ...
    let mockBlobContent = {
      then: jasmine.createSpy('then').and.callFake(cb => {
        ...
        return mockBlobContent;
      }),
      catch: jasmine.createSpy('catch').and.callFake(() => {
        return mockBlobContent;
      }),
      finally: jasmine.createSpy('finally')
    };
    spyOn(storageServer, 'getBlobContent').and.returnValue(mockBlobContent);
